...
[![Release](https://img.shields.io/github/v/release/ghauri01/ShellPilot?label=download&sort=semver)](https://github.com/ghauri01/ShellPilot/releases/latest)
...
| Platform | File | Notes |
|---|---|---|
| **Windows** | `ShellPilot-x.y.z-setup.exe` | Installer, desktop + Start-menu shortcut |
| **Windows (portable)** | `ShellPilot-x.y.z-portable.exe` | Single file, no install, keeps its data beside the `.exe` |
| **macOS** | `ShellPilot-x.y.z-arm64.dmg` | Apple Silicon and Intel builds |
| **Linux** | `ShellPilot-x.y.z-x64.AppImage` | `chmod +x` and run |
| **Linux (deb)** | `ShellPilot-x.y.z-x64.deb` | `sudo apt install ./ShellPilot-*.deb` |
